...
//
ActionFunction::ActionFunction(FunctionData* i_functionData, Modifier i_modifier)
...
ModifiedKey KeySeq::getFirstModifiedKey() const
{
    if ( 0 < m_actions.size() )
    {
        ...
        case Action::Type_keySeq:
            return reinterpret_cast<const ActionKeySeq *>(a)->m_keySeq->getFirstModifiedKey();
        default:
            break;
        }
    }
...
{
    for (KeySeq::Actions::const_iterator i = i_ks.m_actions.begin(); i != i_ks.m_actions.end(); ++i)
        i_ost << **i << _T(" ");
...
Keymap::KeyAssignments &Keymap::getKeyAssignments(const ModifiedKey &i_mk)
{
    ...
    return m_hashedKeyAssignments[i_mk.m_key->getScanCodes()->m_scan % HASHED_KEY_ASSIGNMENT_SIZE];
}


const Keymap::KeyAssignments &Keymap::getKeyAssignments(const ModifiedKey &i_mk) const
{
    ASSERT( 1 <= i_mk.m_key->getScanCodesSize() );
    return m_hashedKeyAssignments[i_mk.m_key->getScanCodes()->m_scan % HASHED_KEY_ASSIGNMENT_SIZE];
}
...
// add modifier
void Keymap::addModifier(Modifier::Type i_mt, AssignOperator i_ao, AssignMode i_am, Key *i_key)
{
    if (i_ao == AO_new)
    {
    ...
    {
        for (ModAssignments::iterator i = m_modAssignments[i_mt].begin(); i != m_modAssignments[i_mt].end(); ++i)
        {
            if ( (*i).m_key == i_key )
            {
...
// search
const Keymap::KeyAssignment *Keymap::searchAssignment(const ModifiedKey &i_mk) const
{
...
void Keymap::adjustModifier(Keyboard &i_keyboard)
{
    for (size_t i = 0; i < NUMBER_OF(m_modAssignments); ++i)
    {
        ...
        else
        {
            ...
            {
                Keyboard::Mods mods = i_keyboard.getModifiers( static_cast<Modifier::Type>(i) );
        ...
        // mod adjust
        for (ModAssignments::iterator mai = m_modAssignments[i].begin(); mai != m_modAssignments[i].end(); ++mai)
        {
...
void Keymap::describe(tostream &i_ost, DescribeParam *i_dp) const
{
    ...
    {
        DescribeParam::DescribedKeymap::iterator i = std::find(i_dp->m_dkeymap.begin(), i_dp->m_dkeymap.end(), this);
    ...
    for (SortedKeyAssignments::iterator i = ska.begin(); i != ska.end(); ++i)
    {
        ...
        DescribeParam::DescribedKeys::iterator j = std::find(i_dp->m_dk.begin(), i_dp->m_dk.end(), i->m_modifiedKey);
...
{
    for (KeymapList::iterator i = m_keymapList.begin(); i != m_keymapList.end(); ++i)
    {
        if ( (*i).getName() == i_name )
            return &*i;
    }
...
{
    for (KeymapList::reverse_iterator i = m_keymapList.rbegin(); i != m_keymapList.rend(); ++i)
...
{
    for (KeySeqList::iterator i = m_keySeqList.begin(); i != m_keySeqList.end(); ++i)
    {
        if ( (*i).getName() == i_name )
            return &(*i);
    }
