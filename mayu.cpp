...
class Mayu
{
    ...
    UINT            m_WM_TaskbarRestart;        ///< window message sent when taskber restarts
...
private:
        ...
        case Notify::Type_name:
            {
                ...
                if (n->m_type == Notify::Type_setFocus)
                    m_engine.setFocus(n->m_hwnd, n->m_threadId, n->m_className, n->m_titleName, false);
        ...
        case Notify::Type_command:
            {
                ...
                m_engine.commandNotify(n->m_hwnd, n->m_message, n->m_wParam, n->m_lParam);
                break;
            }
    ...
    static LRESULT CALLBACK tasktray_wndProc(HWND i_hwnd, UINT i_message, WPARAM i_wParam, LPARAM i_lParam)
    {
        ...
        else
        {
            ...
            case WM_APP_msgStreamNotify:
                {
                    tomsgstream::StreamBuf *log    = reinterpret_cast<tomsgstream::StreamBuf *>(i_lParam);
                    const tstring &         str    = log->acquireString();
                    editInsertTextAtLast(GetDlgItem(This->m_hwndLog, IDC_EDIT_log), str, 65000);
                    log->releaseString();
                }
                return 0;
                        ...
                        case WM_RBUTTONUP:
                            {
                                ...
                                if ( This->m_engine.getIsEnabled() )
                                    CheckMenuItem(hMenuSub, ID_MENUITEM_disable, MF_UNCHECKED | MF_BYCOMMAND);
                                else
                                    CheckMenuItem(hMenuSub, ID_MENUITEM_disable, MF_CHECKED | MF_BYCOMMAND);

                                CHECK_TRUE( SetMenuDefaultItem(hMenuSub, ID_MENUITEM_investigate, FALSE) );
                                ...
                                // show popup menu
                                TrackPopupMenu(hMenuSub, TPM_LEFTALIGN, p.x, p.y, 0, i_hwnd, NULL);
                        ...
                        case ID_MENUITEM_investigate:
                            {
                                ...
                                MoveWindow(This->m_hwndLog, rc1.left, rc1.bottom, rcWidth(&rc1), rcHeight(&rc2), TRUE);
    ...
    void showBanner(bool i_isCleared)
    {
      ...
      #pragma message("\t\t****\t                         - nayuta")
        ...
        _tcsftime( starttimebuf, NUMBER_OF(starttimebuf), _T("%#c"), localtime(&m_startTime) );
     #endif
...
public:
        ...
        , m_startTime(false)
    {
        ...
        m_usingSN                  = wtsRegisterSessionNotification(m_hwndTaskTray, NOTIFY_FOR_THIS_SESSION);
        ...
        m_hwndInvestigate = CreateDialogParam(g_hInst, MAKEINTRESOURCE(IDD_DIALOG_investigate), NULL,
                              dlgInvestigate_dlgProc, (LPARAM) &did);
        CHECK_TRUE( m_hwndInvestigate );

        m_hwndVersion     = CreateDialogParam( g_hInst, MAKEINTRESOURCE(IDD_DIALOG_version),
           ...
           && commonreg.read(_T("layout"), &layout) )
        {
            ...
            else
            {
                ...
                reg.write( _T(".mayu2"), loadString(IDS_109on104Emacs) + tmp + _T(";-DUSE104")
                                                                               _T(";-DUSEdefault")
                                                                               _T(";-DUSE109on104") );
                reg.write( _T(".mayu3"), loadString(IDS_104)           + tmp + _T(";-DUSE104") );
                reg.write( _T(".mayu4"), loadString(IDS_109on104)      + tmp + _T(";-DUSE104")
                                                                               _T(";-DUSE109on104") );
            }
