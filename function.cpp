...
template <class T>
static inline
bool getTypeName(tstring *o_name, T i_type, const TypeTable<T> *i_table, size_t i_n)
{
...
template <class T>
static inline
bool getTypeValue(T *o_type, const tstringi &i_name, const TypeTable<T> *i_table, size_t i_n)
{
    for (size_t i = 0; i < i_n; ++i) {
...
tostream &  operator <<(tostream &i_ost, ToWindowType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_toWindowTypeTable, NUMBER_OF(g_toWindowTypeTable) ) )
        i_ost << name;
    else
...
tostream &  operator <<(tostream &i_ost, GravityType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_gravityTypeTable, NUMBER_OF(g_gravityTypeTable) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_gravityTypeTable, NUMBER_OF(g_gravityTypeTable) );
}
...
tostream &  operator <<(tostream &i_ost, MayuDialogType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_mayuDialogTypeTable, NUMBER_OF(g_mayuDialogTypeTable) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_mayuDialogTypeTable, NUMBER_OF(g_mayuDialogTypeTable) );
}
...
{
    return getTypeValue( o_type, i_name, g_toggleType, NUMBER_OF(g_toggleType) );
}
...
tostream &  operator <<(tostream &i_ost, ModifierLockType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_modifierLockTypeTable, NUMBER_OF(g_modifierLockTypeTable) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_modifierLockTypeTable, NUMBER_OF(g_modifierLockTypeTable) );
}
...
tostream &  operator <<(tostream &i_ost, ShowCommandType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_showCommandTypeTable, NUMBER_OF(g_showCommandTypeTable) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_showCommandTypeTable, NUMBER_OF(g_showCommandTypeTable) );
}
...
tostream &  operator <<(tostream &i_ost, TargetWindowType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_targetWindowType, NUMBER_OF(g_targetWindowType) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_targetWindowType, NUMBER_OF(g_targetWindowType) );
}
...
tostream &operator <<(tostream &i_ost, LogicalOperatorType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_logicalOperatorType, NUMBER_OF(g_logicalOperatorType) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_logicalOperatorType, NUMBER_OF(g_logicalOperatorType) );
}
...
tostream &operator <<(tostream &i_ost, WindowMonitorFromType i_data)
{
    ...
    if ( getTypeName( &name, i_data, g_windowMonitorFromType, NUMBER_OF(g_windowMonitorFromType) ) )
        i_ost << name;
    else
...
{
    return getTypeValue( o_type, i_name, g_windowMonitorFromType, NUMBER_OF(g_windowMonitorFromType) );
}
...
{
    for (std::list<tstringq>::const_iterator i = i_data.begin(); i != i_data.end(); ++i) {
        i_ost << *i << _T(", ");
    }
                          ...
                          RECT *o_rcWindow = NULL, RECT *o_rcParent = NULL)
{
    ...
    *io_twt    = isMdi ? TargetWindowType_mdi : TargetWindowType_overlapped;
    ...
    case TargetWindowType_overlapped:
        ...
        if (o_rcParent) {
            HMONITOR    hm = monitorFromWindow(i_param->m_hwnd, MONITOR_DEFAULTTONEAREST);
            MONITORINFO mi;
            mi.cbSize      = sizeof (mi);
...
/** if the text of the clipboard is
  ...
  */
HGLOBAL Engine::EmacsEditKillLine::makeNewKillLineBuf(const _TCHAR *i_data, int *o_retval)
{
 # if defined(WIN32)
    size_t  len = m_buf.size();
    len += _tcslen(i_data) + 3;

    HGLOBAL hdata = GlobalAlloc( GMEM_MOVEABLE | GMEM_DDESHARE, len * sizeof (_TCHAR) );
    ...
    *dataNew   = _T('\0');
    ...
    if ( 3 <= len && i_data[len - 2] == _T('\r') && i_data[len - 1] == _T('\n') ) {
...
// toggle lock
void Engine::funcToggle(FunctionParam *i_param, ModifierLockType i_lock, ToggleType i_toggle)
{
...
void Engine::funcEditNextModifier(FunctionParam * i_param, const Modifier & i_modifier)
{
    if (!i_param->m_isPressed)
        return;
...
void Engine::shellExecute()
{
    ...
    static const ErrorTable     errorTable[]   = {
        ...
        { SE_ERR_DDEBUSY,           _T("The DDE transaction could not be completed ")
...
public:
    EnumWindowsForSetForegroundWindowParam(const FunctionData_SetForegroundWindow *i_fd)
        : m_fd(i_fd)
        , m_hwnd(NULL)
    {
    }
...
static BOOL CALLBACK enumWindowsForSetForegroundWindow(HWND i_hwnd, LPARAM i_lParam)
{
    ...
    if ( !std::regex_search(tstring(name), what, ep.m_fd->m_windowClassName) ) {
    ...
    if (ep.m_fd->m_logicalOp == LogicalOperatorType_and) {
        ...
        if ( !std::regex_search(tstring(name), what, ep.m_fd->m_windowTitleName) )
            return TRUE;    // match failed
    }
...
void Engine::funcLoadSetting(FunctionParam *i_param, const StrExprArg &i_name)
{
    ...
    PostMessage(m_hwndAssocWindow, WM_APP_engineNotify, EngineNotify_loadSetting, 0);
 #elif defined(__linux__)
    // TODO:
 #elif defined(__APPLE__)
...
void Engine::funcVK(FunctionParam *i_param, VKey i_vkey)
{
    ...
    if      (vkey == VK_LBUTTON && isDown)  mouse_event(MOUSEEVENTF_LEFTDOWN    , 0, 0, 0, 0);
    else if (vkey == VK_LBUTTON && isUp  )  mouse_event(MOUSEEVENTF_LEFTUP      , 0, 0, 0, 0);
    else if (vkey == VK_MBUTTON && isDown)  mouse_event(MOUSEEVENTF_MIDDLEDOWN  , 0, 0, 0, 0);
    else if (vkey == VK_MBUTTON && isUp  )  mouse_event(MOUSEEVENTF_MIDDLEUP    , 0, 0, 0, 0);
    else if (vkey == VK_RBUTTON && isDown)  mouse_event(MOUSEEVENTF_RIGHTDOWN   , 0, 0, 0, 0);
    else if (vkey == VK_RBUTTON && isUp  )  mouse_event(MOUSEEVENTF_RIGHTUP     , 0, 0, 0, 0);
  #if (_WIN32_WINNT >= 0x0500)
    else if (vkey == VK_XBUTTON1 && isDown) mouse_event(MOUSEEVENTF_XDOWN       , 0, 0, XBUTTON1, 0);
    else if (vkey == VK_XBUTTON1 && isUp  ) mouse_event(MOUSEEVENTF_XUP         , 0, 0, XBUTTON1, 0);
    else if (vkey == VK_XBUTTON2 && isDown) mouse_event(MOUSEEVENTF_XDOWN       , 0, 0, XBUTTON2, 0);
    else if (vkey == VK_XBUTTON2 && isUp  ) mouse_event(MOUSEEVENTF_XUP         , 0, 0, XBUTTON2, 0);
  #endif /* _WIN32_WINNT >= 0x0500 */
    else if (isUp || isDown) {
        keybd_event( vkey
            , static_cast<BYTE>( MapVirtualKey(vkey, 0) )
            , (isExtended ? KEYEVENTF_EXTENDEDKEY : 0) | (i_param->m_isPressed ? 0 : KEYEVENTF_KEYUP)
            , 0
        );
    }
 #elif defined(__linux__)
...
void Engine::funcMayuDialog(FunctionParam *i_param, MayuDialogType i_dialog, ShowCommandType i_showCommand)
{
    ...
    PostMessage( getAssociatedWndow(), WM_APP_engineNotify, EngineNotify_showDlg,
                static_cast<LPARAM>(i_dialog) | static_cast<LPARAM>(i_showCommand) );
 #endif
}
...
// show help message
void Engine::funcHelpMessage(FunctionParam *i_param, const StrExprArg &i_title, const StrExprArg &i_message)
{
    if (!i_param->m_isPressed)
        return;
    ...
    PostMessage(getAssociatedWndow(), WM_APP_engineNotify, EngineNotify_helpMessage, doesShow);
 #endif
}
...
void Engine::funcHelpVariable(FunctionParam *i_param, const StrExprArg &i_title)
{
    ...
    m_helpTitle    = i_title.eval();
    m_helpMessage  = buf;
    PostMessage(getAssociatedWndow(), WM_APP_engineNotify, EngineNotify_helpMessage, true);
 #endif
}
...
void Engine::funcWindowRaise(FunctionParam* i_param, TargetWindowType i_twt)
{
    ...
    SetWindowPos(hwnd, HWND_TOP, 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOSIZE);
 #endif
}
...
void Engine::funcWindowLower(FunctionParam *i_param, TargetWindowType i_twt)
{
    ...
    SetWindowPos(hwnd, HWND_BOTTOM, 0, 0, 0, 0, SWP_ASYNCWINDOWPOS | SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOSIZE);
 #endif
}
    ...
    if ( IsZoomed(hwnd) ) {
        ...
        if (target != end) {
            ...
            if (wp.m_mode == WindowPosition::Mode_HV) {
                mode = wp.m_mode = i_isHorizontal ? WindowPosition::Mode_V : WindowPosition::Mode_H;
                              ...
                              int i_dx, int i_dy, TargetWindowType i_twt)
{
    ...
    if (i_gravityType & GravityType_N) y = i_dy + rcd.top;
    if (i_gravityType & GravityType_E) x = i_dx + rcd.right  - rcWidth(&rc);
    if (i_gravityType & GravityType_W) x = i_dx + rcd.left;
    if (i_gravityType & GravityType_S) y = i_dy + rcd.bottom - rcHeight(&rc);
...
public:
    EnumDisplayMonitorsForWindowMonitorToParam(HMONITOR i_hmon)
        : m_hmon(i_hmon)
        , m_primaryMonitorIdx(-1)
        , m_currentMonitorIdx(-1)
    {
    }
};

static BOOL CALLBACK enumDisplayMonitorsForWindowMonitorTo(HMONITOR i_hmon,
                                                           HDC      i_hdc,
                                                           LPRECT   i_rcMonitor,
                                                           LPARAM   i_data)
{
    EnumDisplayMonitorsForWindowMonitorToParam &
            ep = *reinterpret_cast<EnumDisplayMonitorsForWindowMonitorToParam *>(i_data);
...
void Engine::funcWindowIdentify(FunctionParam *i_param)
{
    ...
    if (!ok) {
        ...
        CHECK_TRUE( PostMessage(i_param->m_hwnd, WM_MAYU_MESSAGE, MayuMessage_notifyName, 0) );
    }
 #endif
}
...
void Engine::funcWindowSetAlpha(FunctionParam *i_param, int i_alpha)
{
    ...
    if (i_alpha < 0) {                  // remove all alpha
        for (WindowsWithAlpha::iterator i = m_windowsWithAlpha.begin(); i != m_windowsWithAlpha.end(); ++i) {
            SetWindowLong(*i, GWL_EXSTYLE, GetWindowLong(*i, GWL_EXSTYLE) & ~WS_EX_LAYERED);
            RedrawWindow(*i, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN);
        }
        ...
        if (exStyle & WS_EX_LAYERED) {  // remove alpha
            WindowsWithAlpha::iterator i = std::find(m_windowsWithAlpha.begin(), m_windowsWithAlpha.end(), hwnd);
            ...
            if ( !setLayeredWindowAttributes(hwnd, 0, (BYTE) (255 * i_alpha / 100), LWA_ALPHA) ) {
                Acquire a(&m_log, 0);
...
void Engine::funcClipboardCopy(FunctionParam *i_param, const StrExprArg &i_text)
{
    ...
    HGLOBAL hdataNew   = GlobalAlloc( GMEM_MOVEABLE | GMEM_DDESHARE, (i_text.eval().size() + 1) * sizeof (_TCHAR) );
    if (!hdataNew)
        return;
...
class ParseDirectSSTPData {
    typedef std::match_results<std::regex::const_iterator>  MR;
...
public:
    ...
    bool operator ()(const MR &i_what)
    {
        ...
        if ( member == _T("path") )
            ...
            (*m_directSSTPServers)[id].m_hwnd       = reinterpret_cast<HWND>( _ttoi( value.c_str() ) );
                            ...
                            const std::list<tstringq> & i_headers)
{
    ...
    char *  data = reinterpret_cast<char *>( MapViewOfFile(hfm, FILE_MAP_READ, 0, 0, 0) );
    ...
    std::regex_iterator<std::regex::const_iterator> it(begin, end, getSakura), last;
    for (; it != last; ++it)
    ...
    for (std::list<tstringq>::const_iterator i = i_headers.begin(); i != i_headers.end(); ++i)
    {
    ...
    for (ParseDirectSSTPData::DirectSSTPServers::iterator i = servers.begin(); i != servers.end(); ++i) {
        tcmatch_results what;
...
public:
    ...
    bool load(const tstringq &i_dllName, const tstringq &i_funcName, const tstringq &i_funcParam, tomsgstream &i_log)
    {
