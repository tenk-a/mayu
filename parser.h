...
public:
    ...
    ///
    Token(const tstringi &i_value, bool i_isValueQuoted, bool i_isRegexp = false);
    ///
    Token(Type i_type);
    ...
    bool operator ==(const tstringi &i_str) const { return *this == i_str.c_str(); }
    ///
    ...
    /// paren equal @param i_c '<code>(</code>' or '<code>)</code>'
...
private:
    ...
    const Prefixes* m_prefixes;             ///< string that may be prefix of a token
...
public:
    ...
    /// set string that may be prefix of a token.  prefix_ is not copied, so it must be preserved after setPrefix()
