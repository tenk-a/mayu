...
static bool getFilenameFromRegistry(tstringi *o_name, tstringi *o_filename, Setting::Symbols *o_symbols)
{
    ...
    if (o_filename)
        *o_filename = tstringi(tstring( _tgetenv( _T("HOME") ) ) + "/.mayu");
 #endif
    return true;
}
...
void getHomeDirectories(HomeDirectories *o_pathes)
{
 ...
 #elif defined(__linux__) || defined(__APPLE__)
    const _TCHAR *  home = _tgetenv( _T("HOME") ); // 決め打ち
    if (home)
        o_pathes->push_back(home);
...
bool SettingLoader::getOpenParen(bool i_doesThrow, const _TCHAR *i_name)
{
    ...
    if (i_doesThrow)
        throw ErrorMessage() << _T("there must be `(' after `&") << i_name << _T("'.");
    return false;
}
...
bool SettingLoader::getCloseParen(bool i_doesThrow, const _TCHAR *i_name)
{
    ...
    if (i_doesThrow)
        throw ErrorMessage() << _T("`&")  << i_name << _T("': too many arguments.");
    return false;
}
...
bool SettingLoader::getComma(bool i_doesThrow, const _TCHAR *i_name)
{
    ...
    if (i_doesThrow)
        throw ErrorMessage() << _T("`&")  << i_name << _T("': comma expected.");
    return false;
}
...
void SettingLoader::load_SCAN_CODES(Key *o_key)
{
    for (int j = 0; j < Key::MAX_SCAN_CODES_SIZE && !isEOL(); ++j)
    {
        ...
        while (true)
        {
            ...
            else    throw ErrorMessage() << _T("`") << *t << _T("': invalid modifier.");
        }
    }
}
...
void SettingLoader::load_DEFINE_MODIFIER()
{
    ...
    else throw ErrorMessage() << _T("`") << *t << _T("': invalid modifier name.");
...
void SettingLoader::load_DEFINE_SUBSTITUTE()
{
    ...
    for (AssignedKeys::iterator i = assignedKeys.begin(); i != assignedKeys.end(); ++i)
...
void SettingLoader::load_DEFINE_OPTION()
{
    ...
    {
        if ( *getToken() != _T("!!!") )
            throw ErrorMessage() << _T("there must be `!!!' after `def option delay-of'.");
        if ( *getToken() != _T("=") )
            throw ErrorMessage() << _T("there must be `=' after `def option delay-of !!!'.");
...
// <..._MODIFIER>
Modifier SettingLoader::load_MODIFIER(Modifier::Type i_mode, Modifier i_modifier, Modifier::Type *o_mode)
{
    if (o_mode)
  ...
  continue_loop:
    while ( !isEOL() )
    {
        ...
        for (size_t i = 0; i < NUMBER_OF(map); ++i)
        {
            if (*t == map[i].m_s)
            {
                getToken();
                ...
                if ( static_cast<int>(i_mode) <= static_cast<int>(mt) )
                    throw ErrorMessage() << _T("`") << *t << _T("': invalid modifier at this context.");
...
void SettingLoader::load_KEYMAP_DEFINITION(const Token *i_which)
{
    ...
    m_currentKeymap = m_setting->m_keymaps.add(
        Keymap(type, name->getString(), windowClassName, windowTitleName, NULL, NULL)
    );
    ...
    if (keySeq == NULL)
    {
        ...
        // 参考 : http://gcc.gnu.org/bugs.html#cxx_rvalbind
...
void SettingLoader::load_ARGUMENT(VKey *o_arg)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            if (!vkt->m_name)
                throw ErrorMessage() << _T("`") << *t << _T("': unknown virtual key name.");
            vkey |= vkt->m_code;
            break;
        }
...
void SettingLoader::load_ARGUMENT(TargetWindowType *o_arg)
{
    ...
    if ( getTypeValue( o_arg, t->getString() ) )
        return;
    throw ErrorMessage() << _T("`") << *t << _T("': unknown target window type.");
}
...
void SettingLoader::load_ARGUMENT(WindowMonitorFromType *o_arg)
{
    ...
    if ( getTypeValue( o_arg, t->getString() ) )
        return;
    throw ErrorMessage() << _T("`") << *t << _T("': unknown monitor from type.");
}


// <KEY_SEQUENCE>
KeySeq *SettingLoader::load_KEY_SEQUENCE(const tstringi &i_name, bool i_isInParen, Modifier::Type i_mode)
{
...
void SettingLoader::load_KEY_ASSIGN()
{
    ...
    ModifiedKey     mkey;
    mkey.m_modifier = load_MODIFIER(Modifier::Type_ASSIGN, m_defaultAssignModifier);

    if ( *lookToken() == _T("=") )
    {
        getToken();
        m_defaultKeySeqModifier    = load_MODIFIER(Modifier::Type_KEYSEQ, m_defaultKeySeqModifier);
...
void SettingLoader::load_IF()
{
    ...
    bool    doesRead           = ( (doesSymbolExist && !not_) || (!doesSymbolExist && not_) );
...
bool readFile(tstring *o_data, const tstringi &i_filename)
{
    ...
    //
    if (buf.get()[0] == 0xfeU && buf.get()[1] == 0xffU && sbuf.st_size % 2 == 0)
    // UTF-16 Big Endien
    {
        size_t  size   = static_cast<size_t>(sbuf.st_size) / 2;
        o_data->resize(size);
    ...
    // try UTF-8
    {
        ...
        while (f != end)
        {
            ...
            case STATE_1:
                ...
                else if ( (*f & 0xf0) == 0xe0 )         // 1110xxxx 10xxxxxx 10xxxxxx: 0800 - FFFF
                {
...
void SettingLoader::load(const tstringi &i_filename)
{
    ...
    while (true)
    {
        ...
        catch (WarningMessage &w)
        {
            ...
            {
                Acquire a(m_soLog);
                *m_log  << i_filename << _T("(") << parser.getLineNumber() << _T(") : warning: ") << w << std::endl;
            }
        }
        catch (ErrorMessage &e)
        {
            ...
            {
                Acquire a(m_soLog);
                *m_log  << i_filename << _T("(") << parser.getLineNumber() << _T(") : error: ") << e << std::endl;
            }
...
bool SettingLoader::isReadable(const tstringi& i_filename, int i_debugLevel) const
{
    if ( i_filename.empty() )
        return false;
...
{
    m_defaultKeySeqModifier = m_defaultAssignModifier.release(Modifier::Type_ImeComp);
}
...
bool SettingLoader::load(Setting *i_setting, const tstringi &i_filename)
{
    ...
    if ( !getFilename(i_filename, &path) )
    {
        ...
        {
            throw ErrorMessage() << _T("`") << i_filename << _T("': no such file or other error.");
        }
    }
    ...
    // add default keymap
    m_currentKeymap    = m_setting->m_keymaps.add(
        Keymap(Keymap::Type_windowOr, _T("Global"), _T(""), _T(""), globalDefault, NULL)
    );
...
size_t                 SettingLoader::m_prefixesRefCcount;  ///< reference count of m_prefixes
