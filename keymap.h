...
public:
    ///
    ActionFunction( FunctionData *i_functionData, Modifier i_modifier = Modifier() );
    ///
...
private:
    Actions             m_actions;  ///<
    tstringi            m_name;     ///<
    Modifier::Type      m_mode;     ///< Either Modifier::Type_KEYSEQ or Modifier::Type_ASSIGN
private:
    ///
    void    copy();
    ///
...
public:
    ...
    public:
        ...
        ///
        bool operator <(const KeyAssignment &i_o) const { return m_modifiedKey < i_o.m_modifiedKey; }
    };
...
public:
    ...
    /// add modifier
    void                    addModifier(Modifier::Type i_mt, AssignOperator i_ao, AssignMode i_am, Key *i_key);
    /// search
    ...
    const tstringi &        getName() const { return m_name; }
    /// does same window
    bool                    doesSameWindow(const tstringi i_className, const tstringi & i_titleName);
    /// adjust modifier
    void                    adjustModifier(Keyboard &i_keyboard);
    /// get modAssignments
    const ModAssignments &  getModAssignments(Modifier::Type i_mt) const { return m_modAssignments[i_mt]; }
    /// describe
