...
public:
    ...
    ///
    bool operator !=(const tstringi &i_name) const { return !(*this == i_name); }
    ...
    /// <
    bool operator <(const Key &i_key) const { return getName() < i_key.getName(); }
};
...
public:
    ...
    /// add substitute
    void                addSubstitute(const ModifiedKey& i_mkeyFrom, const ModifiedKey& i_mkeyTo);
    /// get a sync key
    ...
    /// get key iterator
    KeyIterator getKeyIterator() { return KeyIterator(&m_hashedKeys[0], HASHED_KEYS_SIZE); }
};
