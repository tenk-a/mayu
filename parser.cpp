...
Token::Token(Type i_m_type)
    ...
    , m_data(0)
{
    ASSERT(m_type == Type_openParen || m_type == Type_closeParen || m_type == Type_comma);
}
        ...
        while (true) {
            ...
            if (m_prefixes) {
                for (size_t i = 0; i < m_prefixes->size(); i++) {
                    if (_tcsnicmp( tokenStart, m_prefixes->at(i).c_str(), m_prefixes->at(i).size() ) == 0) {
            ...
            // not quoted
            {
                ...
                if (tokenStart == numEnd) {
                    ...
                    o_tokens->push_back( Token( value, tstringi(tokenStart, numEnd - tokenStart) ) );
                    t = numEnd;
                }
                goto continue_getTokenLoop;
            }
